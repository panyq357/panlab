...
现在许多文章都会将自己的原始数据提交到公共数据库中，本篇教程以[这篇文章](https://doi.org/10.1093/plphys/kiac196)为例简单地介绍以下如何下载这些数据。
...
我们可以在 [EBI](https://www.ebi.ac.uk/ena/browser/home) 上搜索这个编号，找到对应的[这个 BioProject 的页面](https://www.ebi.ac.uk/ena/browser/view/PRJNA754696)）。

在 BioProject 的页面中找到 `Generated FASTQ files: FTP` 这一栏目，点击上面 `Download All` 按钮，便可下载下来一个 bash 脚本。这个脚本中写着要下载的文件的链接以及 wget 命令。

在安装好了 WSL 和 Windows Terminal 后（参考：[简单的 WSL 教程]({% post_url 2024-08-12-simple-wsl-tutorial %})），将这个脚本放到要装数据的文件夹里，右键空白处，选择“在终端中打开”，便可在此处启动 WSL，然后键入如下命令：

```bash
bash <下载下来的脚本名>
```

便可用 wget 批量地下载 FASTQ 格式的数据了（如果提示没有 wget，可用 apt 安装）。

然而这样只能一个一个地下，而且由于 wget 不支持断点续传，如果中间断网了，就需要先弄清哪个下了，哪个没下，然后再修改脚本，去掉下过的，再重新运行，比较麻烦。

一个更方便的方法是使用命令行下载工具 aria2 进行下载，aria2 可用 apt 安装。

```bash
sudo apt install aria2
```

aria2 支持从文本文件中直接读取下载地址，所以可以先从 EBI 的脚本中把下载链接复制出来。
...
下载下来的文件名都是 SRR 开头的编号，要知道那个数据对应的是那个样本，可以在 EBI 中 BioProject 的页面上点击 `Show Column Selection`，然后勾上 `sample_alias`，便可显示样本的名字了。

（不同的数据提交者可能在不同的字段里写样品信息，对于其他的数据，找不到的话可多勾选几个其他的字段看看）
